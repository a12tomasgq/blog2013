...
{ // Aqui dentro validaremos todo y grabaremos en la base de datos.
	...
	// Validar password otra expresión regular: ^(?=.*\d)(?=.*[a-zA-Z])(?=.*\w\W).{6,15}$
	if ( !preg_match('/^(?=\w{6,15}$)(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[\w\W]).*/', $_POST['password']) )
	{
		$errores[] = 'The password doesn\'t have a valid format.';
	}
